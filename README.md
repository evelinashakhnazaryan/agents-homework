...
## Готовые материалы и настоящий прогон

25 сентября 2026 года агент успешно обработал все 10 статей через Groq, модель `openai/gpt-oss-120b`, в Google Colab на CPU. Код завершился с `Exit code: 0`, проверка SHA-256, исходных полей, порядка и всех 10 непустых резюме прошла. В трассе 12 вызовов инструментов: 1 чтение, 10 суммаризаций, 1 сохранение. Во время запуска ограничения `429` были обработаны повторами SDK.

- [Выполненный ноутбук с выводами](agents_executed.ipynb) — реальные ячейки, ошибки промежуточных попыток и успешный запуск; для повторного запуска используйте чистый `agents_colab.ipynb`.
- [Результат: исходные статьи и резюме](summaries.json).
- [Трасса успешного агента](summaries.trace.json) и [полный журнал](successful_run.log).
- [Архив всех четырёх сохранённых запусков](runs.zip): успешный и три неудачных, каждый в отдельном каталоге.
- [Краткий отчёт о запусках](run_report.json).

Успешный запуск выполнен на коде commit `cc73a07` с `GROQ_MODEL=openai/gpt-oss-120b`. После этого эта модель установлена по умолчанию; логика агента не изменялась. В метаданных выполненного ноутбука сохранена дополнительная копия архива `run_archive` (ZIP/base64 и SHA-256).

Неудачные попытки сохранены честно: GPT-OSS 20B однажды завершилась без сохранения (после этого добавлена защита в граф), затем API вернул ошибку разбора tool call; Llama 3.3 была недоступна для ключа (`404`). Запуски из ранее утраченной временной среды Colab восстановить нельзя; в архив включены все четыре запуска из текущей сохранённой среды.

Ручной просмотр десяти резюме не выявил противоречий основным фактам и числам исходных статей. Это краткие пересказы: например, в статье 9 опущена часть перечня новых функций CRM. Проверка не является формальной оценкой фактической точности.
...
4. Последняя ячейка скачивает архив с JSON, трассой и `run.log` (stdout/stderr). При ошибке можно выполнить её отдельно: журнал и трасса уже сохранены в уникальном каталоге. Выполненный ноутбук можно сохранить через «Файл → Скачать → Скачать IPYNB».
...
flowchart TD
    ...
    H --> B
    G --> B
    C -->|Нет| K{Подтверждён save_results?}
    K -->|Да| I[END]
    K -->|Нет| L[continue: напоминание модели]
    L --> B
...
Рядом создаётся `имя.trace.json`: версии библиотек, события узлов, вызовы инструментов с аргументами, ответы инструментов, итоговый `success`, а также `finish_reason`, если провайдер его вернул. Трасса сохраняется и при неудаче; в этом случае записывается тип ошибки. Значение API-ключа и окружение не записываются. Трасса содержит заголовки статей и пути; перед публикацией личных данных её нужно проверить.

Успех определяется существованием и проверкой реального файла, а не словами агента. Если LLM ответит без вызова инструмента до сохранения, узел `continue` напомнит о незавершённой задаче. `route` разрешает END только после реального ответа `save_results` с `saved=true`. При исчерпании лимита шагов запуск завершится ошибкой. Файл с результатом сохраняет только MCP-инструмент; постпроверка в `verify_output()` ничего не дописывает.
...
- `build_graph(model, tools)` привязывает MCP-инструменты к модели, строит и компилирует граф. Вложенная `call_model(state)` добавляет системную инструкцию к истории и вызывает модель. `ToolNode` выполняет возвращённые вызовы; `route(state)` направляет tool calls в `ToolNode`, разрешает `END` только после подтверждённого сохранения, иначе переходит в `continue_task(state)`, который добавляет напоминание модели. `tools_condition` используется только для обнаружения tool calls. Ошибки инструментов возвращаются в историю для исправления моделью.
